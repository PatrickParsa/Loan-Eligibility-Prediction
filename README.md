...
The purpose of this project was to build a classification model that minimizes credit risk (the risk of loss resulting from the failure of a borrower to repay the principal and interest owed to the lender). I wanted the model to accomplish this by using customer data to accurately predict whether an application should be approved or not. 


Following the Data Science approach of OSEMN, as outlined and detailed below, I was able to determine a final model that accomplishes the objective while also identifying the key factors that play the biggest part in the decision. 
...
The dataset contained missing values for the loan amount for which I dealt with using the median, this is because I noticed that the distribution is highly skewed with a lot of outliers. I also did some feature engineering and alterations such as adding a column of the total income (applicant + co-applicant income), as well as converting the scale of loan term from months to years. Finally, I encoded the loan status column to make it eligible for modeling. 

### Explore

In this phase, I started off with separating the numerical columns from the categorical ones in order to quickly eyeball the summary statistics of the former as well as checking the percentages for each category for the latter. This gave me a quick idea of how the spread of the data, by showing for example that over 80% of the applicants were male. 

I then undertook bivariate analysis to check the relationships between variables to see if I can find correlations. For example, the following graph shows how the loan amount is positively correlated with the total income which imlpies that the loan amount for higher-income applicants is progressively higher. 
...
I then saw however, that total income may not be a good indicator of an applicant's loan eligibility. The following graph shows no major difference between the income of those customers who are eligible and those who are not. 
...
Before training models, it is important to figure out which performance metric I should focus on the most.  Since I want to minimize financial loss to the company, I want to avoid granting eligiblity to customers that are not eligible and therefore I want to minimize the number of false negatives which means that **recall** is the evaluation criterion that I want to focus on. 
...
For each of these models I also identified the importance of the variables. The below graph shows the feature importances of the random forest. 
...
Following this process, I evaulated these models using the confusion matrix to see how many customers were correctly classified. I paid specific attention to the ineligible customers being predicted a loan approval, because this is the metric that I want to minimize so that financial loss to the company can be avoided. 

## Conclusion

Following the training of these various models, the best model was logistic regression as it provided the highest recall score (66%) for the ineligible applicant class. There is still definitely room for improvement and I will try other algorithms such as xgboost to improve this performance. 

I also concluded that the most important by far for this model was the credit history. Other features that played a part were: 
